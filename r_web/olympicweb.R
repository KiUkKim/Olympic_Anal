...
# 올림픽공원 중심 좌표 37.51980226939681, 127.12273463377676
...
# 중심 좌표
point <- read.csv("predic.csv", fileEncoding = "euc-kr")
geom_poin
center_point <- geom_point(point, aes(x= 127.12778052948829, y=37.52356986500902),
                           size = 80,
                           alpha = 0.2,
                           if(point$차량수 > 350)
                           {
...
center_point2 <- geom_point(df2, mapping = aes(x = rain, y = tmp))

olympic_gate <- ggmap(map) + center_point
olympic_gate
# 웹 로그인 페이지
...
server <- shinyServer(function(input, output,session){
  ...
  output$map <- renderPlotly({
    plot <- (ggmap(map, extent="device") + center_point)
  })
  ...
  autoInvalidate2 <- reactiveTimer(2 * 1000) # 1분마다 실시간 크롤링
